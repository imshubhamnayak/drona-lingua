...
function checkMinimalPairAnswer(button, answer) {
    ...
    if (answer === correct) {
        ...
        // Show celebratory flakes instead of alert
        showCelebration();
        ...
        setTimeout(() => {
            ...
            statusDiv.innerHTML = `Not quite. The correct answer was <strong>${correct}</strong>`;
...
function showCelebration() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        flake.style.animationDuration = (Math.random() * 2.8 + 2.2) + 's';
        flake.style.width = (Math.random() * 9 + 6) + 'px';
        flake.style.height = flake.style.width;
        flake.style.opacity = Math.random() * 0.9 + 0.3;
